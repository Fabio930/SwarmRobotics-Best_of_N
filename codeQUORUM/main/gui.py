...
class PysageGUI(object):
    ...
    def draw_arena(self):
        ...
        for u in range(len(self.utility_id)):
            ...
            if u==0:
                self.w.coords(self.utility_id[u],(node.x+12,node.y1+(node.y2-node.y1)*(1 - node.utility_mean/self.arena.MAX_utility),node.x+18,node.y2))
            else:
    ...
    def paintArena(self):
        ...
        for y in range(len(self.arena.tree.gaussian_kernel)):
            for x in range(len(self.arena.tree.gaussian_kernel[y])):
                color=Color(rgb=cmap(self.arena.tree.gaussian_kernel[y][x]/self.arena.MAX_utility)[:-1])
