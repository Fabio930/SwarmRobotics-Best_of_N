...
class Results:
    ...
    def __init__( self, arena ):
        ...
        if not os.path.exists(self.base ):
            os.mkdir(self.base )
        self.path = self.base+'/K'+ str(self.arena.tree_branches) +'D'+ str(self.arena.tree_depth)+'_'+str(self.arena.num_agents)+'a,'+str(self.arena.max_steps)+'t,'+str(Agent.P_a)+'P_a'
    ...
    def update(self):
        ...
        if self.arena.run_id==1 or self.arena.run_id%10==0:
            ...
            with open(self.Npath+'/location.csv','a') as f:
                ...
                if is_new:
                    writer.writeheader()
                for i in self.arena.agent_position_record.keys():
                    writer.writerow({'id':i,'locations':self.arena.agent_position_record.get(i),'r': round(self.r,2)})
